...
void Vehicle::_mavlinkMessageReceived(LinkInterface* link, mavlink_message_t message)
{
    ...
    case MAVLINK_MSG_ID_DISTANCE_SENSOR:  //*****in PX4 mavlink_messages.cpp  distance_sensor
        _handleDistanceSensor(message);
        break;
    case MAVLINK_MSG_ID_READ_UART_SENSOR:  //*****in PX4 mavlink_messages.cpp read_uart_sensor
...
void Vehicle::_handleReadUartSensor(mavlink_message_t& message)
{
    ...
    if (readUartSensor.current_sensor == -1) {
        batteryFactGroup.BMS_current()->setRawValue(VehicleBatteryFactGroup::_BMS_currentUnavailable);
    } else {
        batteryFactGroup.BMS_current()->setRawValue(readUartSensor.current_sensor);
    }
    if (readUartSensor.temperature_sensor == -1) {
        batteryFactGroup.BMS_temperature()->setRawValue(VehicleBatteryFactGroup::_BMS_temperatureUnavailable);
    } else {
        batteryFactGroup.BMS_temperature()->setRawValue(readUartSensor.temperature_sensor);
    }
    if (readUartSensor.quantity_sensor == -1) {
        batteryFactGroup.BMS_quantity()->setRawValue(VehicleBatteryFactGroup::_BMS_quantityUnavailable);
    } else {
        batteryFactGroup.BMS_quantity()->setRawValue(readUartSensor.quantity_sensor);
    }
    if (readUartSensor.remaining_sensor == -1) {
        batteryFactGroup.BMS_remaining()->setRawValue(VehicleBatteryFactGroup::_BMS_remainingUnavailable);
    } else {
        batteryFactGroup.BMS_remaining()->setRawValue(readUartSensor.remaining_sensor);
    }

    qDebug() << "Receive voltage_sensor" << readUartSensor.voltage_sensor;
    qDebug() << "Receive current_sensor" << readUartSensor.current_sensor;
    qDebug() << "Receive temperature_sensor" << readUartSensor.temperature_sensor;
    qDebug() << "Receive quantity_sensor" << readUartSensor.quantity_sensor;
    qDebug() << "Receive remaining_sensor" << readUartSensor.remaining_sensor;

    /* before new function is get message from px4 to qgc using mavlink */
    /* add new function: send message from qgc to px4 using mavlink */
//    strcpy(readUartSensor.start_charge, "11");
//    strcpy(readUartSensor.stop_charge,"10");
//    int _start_charge = 11;
//    int _stop_charge = 10;
//    readUartSensor.start_charge= 11;
//    readUartSensor.stop_charge = 10;
//    qDebug() << "Send start_charge" << readUartSensor.start_charge;
//    qDebug() << "Send stop_charge" << readUartSensor.stop_charge;

//    mavlink_msg_read_uart_sensor_encode_chan(_mavlink->getSystemId(),
//                                             _mavlink->getComponentId(),
//                                             priorityLink()->mavlinkChannel(),
//                                             &msg,
//                                             &readUartSensor);
//        mavlink_msg_read_uart_sensor_pack_chan(_mavlink->getSystemId(),
//                                                 _mavlink->getComponentId(),
//                                                 priorityLink()->mavlinkChannel(),
//                                                 &msg,
//                                               _start_charge,
...
const char* VehicleBatteryFactGroup::_BMS_quantityFactName =                "BMS_quantity";
const char* VehicleBatteryFactGroup::_BMS_remainingFactName =               "BMS_remaining";
//******************************************************************
...
const int    VehicleBatteryFactGroup::_BMS_quantityUnavailable =          -1.0;
const int    VehicleBatteryFactGroup::_BMS_remainingUnavailable =         -1.0;
//******************************************************************
...
VehicleBatteryFactGroup::VehicleBatteryFactGroup(QObject* parent)
    ...
    , _BMS_quantityFact             (0, _BMS_quantityFactName,              FactMetaData::valueTypeDouble)
    , _BMS_remainingFact            (0, _BMS_remainingFactName,             FactMetaData::valueTypeDouble)
    //******************************************************************
    ...
    , _chargeStateFact              (0, _chargeStateFactName,               FactMetaData::valueTypeUint8)
{
    ...
    _addFact(&_BMS_quantityFact,            _BMS_quantityFactName);
    _addFact(&_BMS_remainingFact,           _BMS_remainingFactName);
    //******************************************************************
    ...
    _BMS_quantityFact.setRawValue       (_BMS_quantityUnavailable);
    _BMS_remainingFact.setRawValue      (_BMS_remainingUnavailable);
    //******************************************************************
